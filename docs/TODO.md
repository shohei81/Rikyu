...
- [x] `cli/debug.ts` — `rikyu debug "<symptom>"` コマンド
- [x] debug 向け mizuya プロンプト（症状 → 仮説 → 確認手順を求める）
- [x] debug 向け teishu プロンプト（水屋の仮説と自らの見解を突き合わせる）
- [x] `/debug [prompt]` スラッシュコマンドの stub 解除
- [x] テスト: debug の水屋振り分けが正しく動く

### Step 13: fix command

- [x] `cli/fix.ts` — `rikyu fix --plan` コマンド（修正計画のみ返す）
- [x] `cli/fix.ts` — `rikyu fix <path> --plan` コマンド
- [x] `cli/fix.ts` — `rikyu fix --patch` コマンド（差分を提示、CLI 承認機構に委ねる）
- [x] `cli/fix.ts` — `rikyu fix --apply` コマンド（承認後に変更適用）
- [x] `collaboration/change-size.ts` — 変更規模推定ロジック
- [x] 変更規模に基づく実行者選択（小→Claude、大→Codex）
- [x] `/fix [prompt]` スラッシュコマンドの stub 解除
- [x] テスト: fix の各モード（plan, patch, apply）の基本動作
- [x] テスト: 変更規模推定と実行者選択

### Step 14: Mode auto-selection

- [x] `session/mode.ts` — collaboration mode の自動選択ロジック（Quick / Standard / Deep）
- [x] `--quick` / `--deep` CLI フラグによる上書き
- [x] config の mode 設定との統合（CLI フラグ > config > 自動選択）
- [x] テスト: mode 自動選択の各ケース

### Step 15: Variable-turn deliberation

- [x] Claude が「水屋に追加質問が必要」と判断した場合の追加ターンフロー
- [x] mode に基づくターン上限の適用（Quick:1, Standard:2, Deep:3）
- [x] 追加ターンの結果を Claude に再度渡すループ処理
- [x] テスト: 追加ターンフローの基本動作

### Step 16: Output options

- [x] `output/json.ts` — `--json` 出力（degraded フラグ、findings、metadata 含む）
- [x] `--verbose` 出力（provider 情報、deliberation summary）
- [x] `output/redaction.ts` — privacy-safe redaction（API キー、トークン等の自動マスク）
- [x] テスト: JSON 出力、verbose 出力、redaction

### Step 17: REPL continuation

- [x] 対話セッション内で review → fix に連続して進む流れ
- [x] 前のターンの結果（MizuyaResponse、findings）を次のターンで参照可能にする
- [x] キャッシュの活用（review 結果を fix で再利用）
- [x] テスト: 連続ターンでのキャッシュ再利用
...
- [x] fix --apply 後に自動で re-review を実行する仕組み
- [x] re-review の結果を前回のレビュー結果と比較する
- [x] 新たな問題が見つかった場合のユーザー通知

### Step 19: Rollback guidance

- [x] fix --apply が失敗した場合のロールバック手順の提示
- [x] git stash / git checkout による復元ガイダンス
- [x] 変更適用前の状態を記録する仕組み
...
- [x] MizuyaResponse → SARIF v2.1.0 変換ロジック
- [x] `--sarif` CLI フラグ
- [x] SARIF 出力のバリデーション（GitHub Code Scanning 互換性確認）

### Step 22: CI-friendly mode

- [x] `--ci` フラグ（非対話、進捗表示なし、exit code で結果を返す）
- [x] quiet mode（finding がなければ何も出力しない）
- [x] CI 環境の自動検知（CI=true 環境変数）

### Step 23: GitHub Actions sample

- [x] GitHub Actions workflow サンプルファイル
- [x] PR コメントへの結果投稿スクリプト
- [x] SARIF アップロードの設定例

### Step 24: Policy profiles

- [x] チーム向け設定プロファイル（厳格モード、寛容モード等）
- [x] .rikyu/policy.json によるチーム共通ルール定義
- [x] プロファイルの切り替え機構
