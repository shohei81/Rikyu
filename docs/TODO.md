...
- [ ] `/help` — コマンド一覧表示
- [ ] `/exit` — セッション終了
- [ ] `/review [prompt]` — review 開始（task 確定、分類スキップ）
- [ ] `/ask [prompt]` — ask 開始（task 確定、分類スキップ）
- [ ] `/debug [prompt]` — debug 開始（Phase 0 では stub / エラーメッセージ）
- [ ] `/fix [prompt]` — fix 開始（Phase 0 では stub / エラーメッセージ）
...
- [ ] スラッシュコマンド時の分類スキップ
- [ ] セッションの自動保存（各ターン後にスナップショット更新）
- [ ] `rikyu --resume` CLI オプション
- [ ] テスト: slash commands の全テストケース（コマンドのみ、コマンド+プロンプト、未知コマンド、空入力、スラッシュなし）
- [ ] テスト: task routing の全テストケース（review→通す、debug→通す、explain→通す、fix→通さない、会話→通さない、スラッシュ→分類スキップ）
